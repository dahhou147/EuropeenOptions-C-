...
std::vector<DailyBar> fetch_alpha_vantage_daily_adjusted(const std::string& symbol,
    ...
    std::string url_adj =
        ...
        "&symbol=" +
        symbol + "&outputsize=" + outputSize + "&datatype=json&apikey=" + apiKey;
    std::string url_daily =
    ...
    body = perform_request(url_daily);
    {
        ...
        if (root.isMember("Error Message"))
            throw std::runtime_error("Alpha Vantage error: invalid request/symbol");
        if (root.isMember("Note"))
            throw std::runtime_error("Alpha Vantage rate limit hit. Try later.");
        const Json::Value& series = root["Time Series (Daily)"];
        if (!series.isObject()) {
        ...
        for (const std::string& date : dates) {
            const Json::Value& bar = series[date];
            if (!bar.isObject() || !bar.isMember("4. close"))
                continue;
